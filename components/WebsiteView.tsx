...
export default function WebsiteView({ view, onRemove, onTypeChange }: WebsiteViewProps) {
  ...
  const scale = view.type === 'desktop' ? 0.4 : view.type === 'tablet' ? 0.5 : 0.8

  return (
    <Card className="w-full overflow-hidden" style={{ 
      width: `${viewDimensions[view.type].width * scale + 40}px`,
      height: `${viewDimensions[view.type].height * scale + (showCustomSize ? 220 : 180)}px`,
    }}>
      ...
      <CardContent className="space-y-4 p-2">
          ...
          <div className="space-y-2">
            <div className="flex items-center space-x-2">
              ...
              <Input
                ...
                className="w-20"
              />
              <Button onClick={handleCustomDimensions} size="sm">Set</Button>
            </div>
          </div>
        )}
        ...
        <div className="overflow-hidden bg-background" style={{
